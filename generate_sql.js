...
sql += `INSERT INTO restaurants (id, slug, name, theme_config, whatsapp_number) VALUES ('${restaurantId}', 'shawarma-nazo-land', 'شاورما نازو لاند', '{"primary_color": "#d97706", "secondary_color": "#ffffff", "font": "Inter"}'::jsonb, '+9647701234567');\n\n`;

categories.forEach((cat, index) => {
  const catId = generateUUID();
  sql += `INSERT INTO categories (id, restaurant_id, name, order_index) VALUES ('${catId}', '${restaurantId}', '${cat.name}', ${index});\n`;
  
  cat.items.forEach(item => {
    const prodId = generateUUID();
    sql += `INSERT INTO products (id, restaurant_id, category_id, title, price, description, image_url, is_available) VALUES ('${prodId}', '${restaurantId}', '${catId}', '${item.name}', ${item.price}, '${item.desc}', '${item.img}', true);\n`;
  });
  sql += '\n';
});

sql += `-- COPY THIS RESTAURANT ID TO LINK IT TO YOUR USER ACCOUNT:\n`;
sql += `-- ${restaurantId}\n`;

fs.writeFileSync('seed-nazo.sql', sql);
console.log('SQL file created: seed-nazo.sql');
