...
// In Super Menu we might not save orders to DB yet, or we use a Supabase action. For now, let's just trigger WhatsApp directly like the old one, but bypass DB if it fails, or implement a simple server action later.

const CheckoutModal = memo(({ cart, totalPrice, onClose, onSuccess, restaurant }: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const whatsappNumber = restaurant?.whatsapp_number?.replace(/\+/g, '').replace(/ /g, '') || "9647719933131";
    const itemsList = cart.map((c: any) => {
      const itemName = `${c.product.title}${c.selectedSize ? ` (${c.selectedSize})` : ''}`;
      const itemPrice = c.selectedPrice || c.product.price;
      return `📦 ${c.quantity}x ${itemName} (${(itemPrice * c.quantity).toLocaleString()} د.ع)`;
    }).join('\n');
    
    const message = `*طلب جديد من تطبيق ${restaurant?.name || 'Shawarma Nazo Land'}* 🌯🔥\n\n` +
      `*التفاصيل:*\n${itemsList}\n\n` +
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center sm:items-center p-4 pb-0 sm:p-6 animate-fade-in text-right" dir="rtl">
      {/* Dynamic Backdrop */}
      <div 
      ...
      <div className="glass bg-[#050505]/95 w-full max-w-md rounded-t-[3rem] sm:rounded-[3rem] shadow-[0_0_50px_rgba(0,0,0,0.5)] relative z-10 border border-white/10 flex flex-col max-h-[90vh] overflow-hidden transform transition-all duration-500">
        
        {/* Glow Decor */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-48 h-1 bg-gradient-to-r from-transparent via-[#00ca72] to-transparent opacity-50"></div>
        ...
        <div className="p-6 md:p-8 pt-2 overflow-y-auto no-scrollbar">
          ...
          <div className="bg-white/[0.03] rounded-[2rem] p-5 mb-8 border border-white/5 relative overflow-hidden">
             <div className="absolute top-0 right-0 w-16 h-16 bg-[#ffcc00]/5 blur-2xl rounded-full"></div>
             <div className="flex justify-between items-center mb-1">
                <span className="text-xs font-bold text-gray-500 uppercase">المبلغ الإجمالي</span>
                <span className="text-[10px] font-black text-[#ffcc00] bg-[#ffcc00]/10 px-2 py-0.5 rounded-lg">قيد الدفع</span>
             </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative group/input">
                <div className="absolute inset-y-0 right-4 flex items-center pointer-events-none text-gray-500 group-focus-within/input:text-[#00ca72] transition-colors">
                ...
                <input
                  ...
                  className="w-full bg-white/[0.03] border border-white/5 rounded-[1.5rem] pr-12 pl-4 py-4 text-white placeholder-gray-600 focus:outline-none focus:border-[#00ca72]/50 focus:bg-white/[0.05] transition-all font-bold text-base md:text-sm"
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group/input">
                 <div className="absolute top-4 right-4 text-gray-500 group-focus-within/input:text-[#00ca72] transition-colors">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-white/[0.03] border border-white/5 rounded-[1.5rem] pr-12 pl-4 py-4 text-white placeholder-gray-600 focus:outline-none focus:border-[#00ca72]/50 focus:bg-white/[0.05] transition-all font-bold text-base md:text-sm resize-none"
                />
              </div>
            </div>
            ...
            <button
              ...
              className={`group relative w-full py-5 rounded-[1.8rem] font-black text-lg transition-all active:scale-[0.98] overflow-hidden ${
                isSubmitting ? "bg-gray-800 text-gray-500 cursor-not-allowed border border-white/5" : "bg-[#00ca72] text-white"
              }`}
              style={!isSubmitting ? { boxShadow: '0 0 20px rgba(0,202,114,0.4)' } : {}}
            >
          ...
          <p className="text-center text-[10px] font-bold text-gray-600 uppercase tracking-widest mt-8">
            {restaurant?.name || 'Shawarma Nazo Land'} • Premium Food Experience
          </p>
        </div>
      </div>
    </div>
